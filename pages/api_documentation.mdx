...
Currently, this endpoint processes a request even if all fields are empty. This results in a new record with `null` values for `name`, `salary`, and `age`. To prevent the creation of records without essential information, a validation should be added. This would ensure that any request to this endpoint fails with a if `name`.
